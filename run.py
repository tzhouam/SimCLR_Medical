...
from models.models_simclr import ResNetSimCLR, DenseNetSimCLR
...
def main():
    ...
    dataset = ContrastiveLearningDataset(args.data,args,mode='pretrain',usage='train')
    # print(dataset.total_imgs)
    train_dataset = dataset.get_dataset(args.dataset_name, args.n_views)
    # print(train_dataset.total_imgs,train_dataset.root)
    train_loader = torch.utils.data.DataLoader(
        train_dataset, batch_size=args.batch_size, shuffle=True,
        num_workers=args.workers, pin_memory=True, drop_last=True)

    if args.arch.startswith('resnet'):
        model = ResNetSimCLR(base_model=args.arch, out_dim=map_label_type_to_num[args.label_type])
    elif args.arch.startswith('densenet'):
        model = DenseNetSimCLR(base_model=args.arch, out_dim=map_label_type_to_num[args.label_type])
    else:
        raise InvalidBackboneError(
                "Invalid backbone architecture. Only support ResNet and DenseNet.")
